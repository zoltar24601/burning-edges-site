// ============================================================
// Every-5-min engine for the World Cup Animal Print Edition. Same loop as the
// other packs:
//   1. append new on-chain Animal Print SALES -> pack_sales
...
//
// The pack crafts 2026-09-09 with no secondary sales yet, so until the cards
// trade this publishes the modeled seed each run. Deploy-safe: falls back to
// the static value map + full runs so it works before any seeding.
// Env (Netlify): SUPABASE_URL, SUPABASE_SERVICE_KEY.
// ============================================================
import { recomputeAnimalPrint } from "../../tools/animalprint-recompute.mjs";
import { repriceAnimalPrint } from "../../tools/animalprint-reprice.mjs";
import staticMap from "../../tools/animalprint-values.json";

// Every 5 min: on craft day packs rip fast and the page reads this snapshot --
// hourly would leave it lagging behind live card_remaining. Recompute + reprice
// are cheap.
export const config = { schedule: "*/5 * * * *" };
const PRODUCT = "animalprint";
// Animal Print SKUs share prefix packcard-2332 with the main set + Floral, so
// we can't filter by prefix -- the chain-sales query is built from the exact
// seeded pack_values SKUs (see below).
const notable = (o, n) => Math.abs(n - o) >= 50 && Math.abs(n - o) / Math.max(o, 1) >= 0.15;
...
export default async () => {
  ...
  try {
    // 1) value map: pack_values if seeded, else the static seed (keyed by sku_base)
    ...
    // 2) append new on-chain Animal Print SALES (price>0) -> pack_sales. Query by
    //    the exact seeded SKUs so we don't pull the whole main set.
    ...
    const { newValues, moves } = repriceAnimalPrint(vrows, sales);
    if (seeded && moves.length) {
      ...
      const feed = moves.filter(m => notable(m.old, m.new)).map(m => ({ product: PRODUCT, scope: `${m.athlete} ${m.cardset.replace("Base Choice Prizms ", "")} /${m.run}`, headline: `${m.athlete} ${m.cardset.replace("Base Choice Prizms ", "")} repriced off recent sales`, detail: `$${m.old.toLocaleString()} -> $${m.new.toLocaleString()}`, old_value: m.old, new_value: m.new }));
    ...
    const payload = recomputeAnimalPrint(remaining, valueMap);

    const cur = await (await fetch(`${SB_URL}/rest/v1/animalprint_snapshots?select=id&published=eq.true&order=computed_at.desc&limit=1`, { headers: H })).json();
    if (!Array.isArray(cur) || !cur.length) return resp(409, { error: "no published animalprint snapshot yet (run migration 010 + publish)" });
    await fetch(`${SB_URL}/rest/v1/animalprint_snapshots?id=eq.${cur[0].id}`, { method: "PATCH", headers: H, body: JSON.stringify({ payload, pack_book_ev: payload.pack_ev.pack_book_ev, updated: payload.updated, computed_at: new Date().toISOString() }) });
