...
export async function handler(event) {
  ...
  try {
    ...
    // Decode compact form: entries store {n, l:[indices]} + a shared dict.
    // Fall back to legacy {name, lineup:[names]} if an old row is present.
    const dict = Array.isArray(ev.dict) ? ev.dict : null;
    const decodeEntry = e => {
      if (Array.isArray(e.lineup)) return e.lineup;            // legacy
      if (dict && Array.isArray(e.l)) return e.l.map(i => dict[i]); // compact
      return [];
    };

    const allUnmatched = new Set();
    const dists = ev.contests.map(c => {
      const entries = (c.entries || []).map(e => ({ lineup: decodeEntry(e) }));
      const r = (mode === "final") ? fieldFinal(entries, lookup) : fieldPredicted(entries, lookup);
