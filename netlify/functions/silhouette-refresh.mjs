// ============================================================
// HOURLY engine for Silhouette Basketball. Same loop as the NFL engine:
//   1. append new on-chain sales (chain_events, prefixes packcard-2217 + 2305) -> pack_sales
...
// Cooper Flagg stays an owner floor (1/1 $40k, /10 $7,500). Falls back to the
// static value map until pack_values is seeded (deploy-safe).
// Env (Netlify): SUPABASE_URL, SUPABASE_SERVICE_KEY.
// ============================================================
import { recomputeSilhouette } from "../../tools/silhouette-recompute.mjs";
import { repriceSil } from "../../tools/silhouette-reprice.mjs";
import staticMap from "../../tools/silhouette-values.json";

export const config = { schedule: "@hourly" };
const PRODUCT = "silhouette";
const EV_FILTER = "or=(sku_base.like.packcard-2217*,sku_base.like.packcard-2305*)";
...
export default async () => {
  ...
  try {
    ...
    // 2) append new on-chain sales -> pack_sales
    ...
    const { newValues, moves } = repriceSil(vrows, sales);
    ...
    await fetch(`${SB_URL}/rest/v1/silhouette_snapshots?id=eq.${cur[0].id}`, { method: "PATCH", headers: H, body: JSON.stringify({ payload, pack_book_ev: payload.pack_ev.pack_book_ev, updated: payload.updated, computed_at: new Date().toISOString() }) });
